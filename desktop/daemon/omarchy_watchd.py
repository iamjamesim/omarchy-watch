...
class WatchDaemon:
    ...
    def refresh_devices(self) -> bool:
        ...
        if (not paired and self.pending_passkey is not None and
            ...
            message = "Ready to pair" if not paired else "Connected" if connected else "Paired"
        self.write_state(
            status=status,
        ...
        if paired:
            if connected and not self.state.get("lastSynced", 0):
                self.schedule_profile_sync()
            elif not connected and not self.connect_inflight:
    ...
    def on_identity_read(self, raw_identity) -> None:
        ...
        if magic != b"OW" or not protocol_min <= PROTOCOL_VERSION <= protocol_max:
            self.write_inflight = False
    ...
    def profile_payload(self) -> bytes:
        ...
        revision = epoch & 0xFFFFFFFF
        cycle = self.desktop_hour_cycle()
        return struct.pack(
            "<2sBBIq hBB16s".replace(" ", ""),
            b"OW", PROTOCOL_VERSION, 1, revision, epoch, offset, cycle, 0, self.host_id,
        )
    ...
    def on_profile_written(self) -> None:
        ...
        self.write_state(
            ...
            message="Time and preferences are up to date",
        )
